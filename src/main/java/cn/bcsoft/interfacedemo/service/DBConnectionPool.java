 ...
 * @Description 数据库连接池
 * Created by blackchen on 2020/9/27 11:34
 */
//////////////////////////////// 数据库连接池类 DBConnectionPool.java ////////////////////////////////////////
...
/*
 ...
 = new DBConnectionPool("com.microsoft.jdbc.sqlserver.SQLServerDriver"
...
public class DBConnectionPool {
	private String jdbcDriver = DBInfoConfig.getDriver(); // 数据库驱动
	private String dbUrl = DBInfoConfig.getUrl(); // 数据 URL
	private String dbUsername = DBInfoConfig.getUsername(); // 数据库用户名
	private String dbPassword = DBInfoConfig.getPassword(); // 数据库用户密码
	private String testTable = DBInfoConfig.getTablename(); // 测试连接是否可用的测试表名，默认没有测试表
	...
	private void createConnections(int numConnections) throws SQLException {
		...
		for (int x = 0; x < numConnections; x++) {
			// 是否连接池中的数据库连接的数量己经达到最大？最大值由类成员 maxConnections 指出，
	...
	private Connection newConnection() throws SQLException {
		...
		// 最大客户连接数目
		if (connections.size() == 0) { //connections.size()==0 表示目前没有连接己被创建
	...
	/**
	 * 如果当前没有可用的数据库连接，本函数则根据incrementalConnections 设置 的值创建几个数据库连接，并放入连接池中。
	...
	private boolean testConnection(Connection conn) {
		try {
			// 判断测试表是否存在
			if (testTable.equals("")) {
				// 如果测试表为空，试着使用此连接的 setAutoCommit() 方法来判断连接否可用（此方法只在部分数据库可用，如果不可用,抛出异常）。
				// 注意：使用测试表的方法更可靠
